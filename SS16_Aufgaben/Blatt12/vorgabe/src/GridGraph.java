...
public class GridGraph extends DiGraph {
	...
	Picture toPicture(List<Node> path) {
		...
		RGBColor[][] pixels = new RGBColor[columns][rows]; //image is width=columns, then height=rows
		...
		for (CellNode n : cellnodes) {
			RGBColor pixelcolor;
			if (n.status != CellNode.WHITE) {
				pixelcolor = gray;
			} else {
				pixelcolor = white;
			}
			pixels[n.getColumn()][n.getRow()] = pixelcolor;
		}

		// Paint path red
		if (path != null) {
			for (Node p : path)
				pixels[getCellNode(p).getColumn()][getCellNode(p).getRow()] = red;
		}
	...
	public Node addNode() {
		System.out
				.println("You need to supply a row and column index. Not adding any node.");
		return null;
	}
	...
	public CellNode getCellNode(int id) {
		return getCellNode(nodes.get(id)); // Simply downcast, as we ensure that
											// we only add CellNodes
	}

	public CellNode getCellNode(int i, int j) {
		if (i < 0 || j < 0 || i >= rows || j >= columns) {
			return null;
		} // that node cannot exist
		int id = CellNode.computeID(i, j);
		CellNode n;
		try {
			n = getCellNode(nodes.get(id)); // Simply downcast, as we ensure
											// that we only add CellNodes
		} catch (Exception e) {
			n = null;
		}
		return n;
	...
	public int getNodeRow(Node n) {
		CellNode cellnode = (CellNode) n; // Downcast, as we ensure that we only
											// add CellNodes
	...
	public void populateAStar(CellNode startCNode, CellNode targetCNode) {
		this.resetState();
		if (startCNode == null || !cellnodes.contains(startCNode)) {
			return;
		}
		if (targetCNode == null || !cellnodes.contains(targetCNode)) {
			return;
		}
	...
	public List<Node> getShortestPathAStar(int startNodeID, int targetNodeID) {
		return getShortestPathAStar(getCellNode(startNodeID),
				getCellNode(targetNodeID));
	}
