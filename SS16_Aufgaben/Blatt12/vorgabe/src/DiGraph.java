...
public class DiGraph implements Graph {
	...
	// ------------------------------------------------------------------------
		...
		public Graph toMinSpanTree() {
			...
			// sort edges ascending by weights:
			// create PrioQ
			PriorityQueue<Edge> edges = new PriorityQueue<Edge>();
			// collect edges in PrioQ
			for (Node n : nodes.values()) {
				for (Edge e : n.edges) {
					edges.add(e);
				}
			...
			// put each node as its own set
			UnionFindSet<Integer> ufd = new UnionFindSet<Integer>();
			
			DiGraph mst = new DiGraph(); // result graph
			for (int i = 0; i < nodes.size(); i++) {
				Node newNode = mst.addNode();
				ufd.add(newNode.getID());

			}
					
			// use greedy Kruskal to join them
			Edge e;
			Node start, end;
			while ((e = edges.poll()) != null) { // get ~lightest edge
				start = e.startnode;
				end = e.endnode;
				
				// here a ufd returns a reference to a Set.
				// Partitions with the same elements should also be identical, so both equals() and == should work here
	            // In terms of efficiency, we probably want to use identity (==)
				if (ufd.getRepresentative(start.getID()) != ufd.getRepresentative(end.getID())) { // does the edge join 2 graphs?
					// then merge them
					ufd.union(start.getID(), end.getID());
					// and add edge in result-graph
					mst.addEdge(start.getID(), end.getID(), e.weight);
					System.out.println("adding edge between " + start + " and " + end);
				}
			}
			
			if (!mst.nodes.isEmpty())
				return mst;
